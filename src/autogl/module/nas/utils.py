    ...
    def apply(m):
        for name, child in m.named_children():
            if isinstance(child, type_name):
                setattr(m, name, init_fn(child))   # m.name = init_fn(child)
                modules.append((child.key, getattr(m, name)))  # child.key,m.name
            else:
                apply(child)
...
def serve_replace_input_choice(root_module, init_fn, device,modules=None):

    return serve_replace_module_with_type(
        root_module, init_fn, (InputChoice, nn.InputChoice), modules,device
    )

def serve_replace_module_with_type(root_module, init_fn, type_name, modules,device):
    if modules is None:
        modules = []
    ...
    def apply(m):
        for name, child in m.named_children():
            if isinstance(child, type_name):
                setattr(m, name, init_fn(child,device))   # m.name = init_fn(child)
                modules.append((child.key, getattr(m, name)))  # child.key,m.name
            else:
                apply(child)
...
def serve_replace_layer_choice(root_module, init_fn, device,modules=None):
    """
    Replace layer choice modules with modules that are initiated with init_fn.
    ...
    """
    return serve_replace_module_with_type(
        root_module, init_fn, (LayerChoice, nn.LayerChoice), modules,device
    )

def pf_replace_layer_choice(root_module, init_fn,operations, device,modules=None):
    """
    Replace layer choice modules with modules that are initiated with init_fn.
    ...
    List[Tuple[str, nn.Module]]
        A list from layer choice keys (names) and replaced modules.
    """
    return pf_replace_module_with_type(
        root_module, init_fn, operations, (LayerChoice, nn.LayerChoice), modules,device
    )

def pf_replace_module_with_type(root_module, init_fn,operations, type_name, modules,device):
    ...
    def apply(m):
        for name, child in m.named_children():
            if isinstance(child, type_name):
                ...
                setattr(m, name, init_fn(child,device,choose_letter))   # m.name = init_fn(child)
                modules.append((child.key, getattr(m, name)))  # child.key,m.name
            else:
                apply(child)
