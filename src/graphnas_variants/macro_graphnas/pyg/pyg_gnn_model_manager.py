...
from src.graphnas.gnn_model_manager import CitationGNNManager, evaluate
from src.graphnas_variants.macro_graphnas.pyg.pyg_gnn import GraphNet
from src.graphnas.utils.label_split import fix_size_split
...
class GeoCitationManager(object):
    def __init__(self,data,cfg):
        super(GeoCitationManager, self).__init__()
        # if hasattr(args, "supervised"):
        #     self.data = load_data(args.dataset, args.supervised)
        # else:
        #     self.data = load_data(args.dataset)

        self.in_feats = cfg.model.input_shape
        self.n_classes = cfg.model.num_classes
        self.in_drop = cfg.model.dropout
        self.device = cfg.device
        self.data = data
        # device = torch.device('cuda' if args.cuda else 'cpu')
        # self.data.to(cfg.device)

    def build_gnn(self, actions):
        model = GraphNet(actions, self.in_feats, self.n_classes, drop_out=self.in_drop, multi_label=False,
