...
class NodeFullBatchTrainer(GeneralTorchTrainer):
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        # logit = ctx.model(batch.x,batch.edge_index)[batch['train_mask']]
        pred = ctx.model(batch.x,batch.edge_index)[batch['{}_mask'.format(ctx.cur_split)]]
    ...
    def _hook_on_batch_forward(self, ctx):
        if ctx.cur_split == 'train':
            ...
            if self.is_NeighborSampler:
                ...
                if ctx.cfg.federate.method == 'fedsageplus':  # in ['pfgnas-random','pfgnas-evo','pfgnas','pfgnas-one']:
                    edge_index = []
                    for edge, _, size in adjs:
                        edge_index.append(edge)
                    edge_index = torch.cat(edge_index, dim=1)
                    pred = ctx.model(ctx.data['data'].x[n_id], edge_index)
                    label = ctx.data['data'].y[n_id]
                    ctx.batch_size = len(label)
                else:
                    pred = ctx.model(ctx.data['data'].x[n_id], adjs)
