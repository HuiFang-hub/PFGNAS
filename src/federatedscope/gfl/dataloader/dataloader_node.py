...
from src.FedPUB.data.generators.utils import   torch_save
...
def load_nodelevel_dataset(config=None):
    ...
    num_classes = len(set(global_dataset.data.y.tolist()))
    dataset = [ds for ds in dataset]
    client_num = min(len(dataset), config.federate.client_num
                     ) if config.federate.client_num > 0 else len(dataset)
    config.merge_from_list(['federate.client_num', client_num])
    config.merge_from_list(['model.num_classes', num_classes])
    config.merge_from_list(['model.input_shape', tuple(global_dataset.data.x.shape)])
    # get local dataset
    data_path = os.path.join(config.data.root, f'{name}')
    data_dict = dict()
    for client_idx in range(1, len(dataset) + 1):
        ...
        if config.data.splitter == 'lda':
            ...
            torch_save(data_path, f'{config.data.mode}/{config.federate.client_num}_{alpha}/partition_{client_idx - 1}.pt', {
                'client_data': local_data,
                'client_id': client_idx - 1
            })
        else:
            torch_save(data_path, f'{config.data.mode}/{config.federate.client_num}/partition_{client_idx-1}.pt', {
                'client_data': local_data,
                'client_id': client_idx-1
            })
    ...
    if global_dataset is not None:
        ...
        if config.data.splitter == 'lda':
            alpha = config.data.splitter_args[0]['alpha']
            torch_save(data_path, f'{config.data.mode}/{config.federate.client_num}_{alpha}/partition_global.pt', {
                'client_data': global_graph,
                'client_id': 'global'
            })
        else:
            torch_save(data_path, f'{config.data.mode}/{config.federate.client_num}/partition_global.pt', {
                'client_data':global_graph,
                'client_id': 'global'
            })
