...
class fedDarts_trainer(GeneralTorchTrainer):
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        if ctx.cur_split=='train' or ctx.cur_split=='val':
            ...
            pred = ctx.model(batch)[batch['val_mask']]
            label = batch.y[batch['val_mask']]
            ctx.arch_loss_batch = CtxVar(ctx.criterion(pred, label), LIFECYCLE.BATCH)
            # ctx.model.train()
...
def  call_fedDarts_trainer(trainer_type):
    if trainer_type == 'fedDarts_trainer':
        trainer_builder = fedDarts_trainer
    else:
        trainer_builder = None

    return trainer_builder

register_trainer('fedDarts_trainer', call_fedDarts_trainer)