...
from src.federatedscope.gfl.model.fedsageGod import LocalSage_Plus, FedSage_Plus
from src.federatedscope.gfl.fedsageGod.utils import GraphMender, HideGraph
from src.federatedscope.gfl.fedsageGod.trainer import LocalGenTrainer, \
    FedGenTrainer

logger = logging.getLogger(__name__)


class FedSageGodServer(Server):
    def __init__(self,
                 ID=-1,
                 state=0,
                 config=None,
    ...
    def _register_default_handlers(self):
        ...
        self.register_handlers('gad_para', self.callback_funcs_model_para)
...
class FedSageGodClient(Client):
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(FedSageGodClient,
              self).__init__(ID, server_id, state, config, data, model, device,
                             strategy, *args, **kwargs)
        self.data = data
        # from collections import Counter
        # test = Counter(self.data['data'].ay.tolist())
        ...
        self.hide_test_data = HideGraph(self._cfg.fedsageplus.hide_portion)(
            data['test_data'])
        # Convert to `ClientData`
        self.hide_test_data = ClientData(self._cfg,
                                    train=self.hide_test_data,
                                    val=self.hide_test_data,
                                    test=self.hide_test_data,
                                    data=self.hide_test_data)
        self.device = device
        ...
        self.register_handlers('gad_para', self.callback_funcs_for_model_para)
        ...
        self.register_handlers('setup', self.callback_funcs_for_setup_pygod)
    ...
    def callback_funcs_for_setup_pygod(self, message: Message):
        round, sender, _ = message.state, message.sender, message.content
        # from collections import Counter
        # test = Counter(self.hide_data['data'].ay.tolist())
        # teste2 = Counter(self.data['data'].ay.tolist())
        self.filled_data = GraphMender(
            model=self.fedgen,
            impaired_data=self.hide_data['data'].cpu(),
            original_data=self.data['data'])
        self.filled_test_data = GraphMender(
            model=self.fedgen,
            impaired_data=self.hide_test_data['data'].cpu(),
            original_data=self.data['test_data'])
        # subgraph_sampler = NeighborSampler(
        #     self.filled_data.edge_index,
        #     sizes=[-1],
        #     batch_size=4096,
        ...
        fill_dataloader = {
            'data': self.filled_data,
            'test_data':self.filled_test_data
            # 'train': NeighborSampler(
            #     self.filled_data.edge_index,
            #     node_idx=self.filled_data.train_idx,
            #     sizes=self._cfg.dataloader.sizes,
        ...
        # test = Counter(self.filled_data['data'].ay.tolist())
        # teste2 = Counter(self.filled_test_data['data'].ay.tolist())
        self._cfg.merge_from_list(
            ['dataloader.batch_size', self.sage_batch_size])
        if self._cfg.model.type == "cola":
            from src.federatedscope.gfl.fedgod.trainer import fedgod_trainer
            self.trainer_gad = fedgod_trainer(model=self.clf,
                                          data=fill_dataloader,
                                          device=self.device,
                                          config=self._cfg,
                                          only_for_eval=False,
                                          monitor=self._monitor)
        elif self._cfg.model.type == "anemone":
            from src.federatedscope.gfl.fedAnemone.trainer import fedanemone_trainer
            self.trainer_gad = fedanemone_trainer(model=self.clf,
                                          data=fill_dataloader,
                                          device=self.device,
                                          config=self._cfg,
                                          only_for_eval=False,
            ...
            self.trainer_gad = NodeMiniBatchTrainer(self.clf,
                                                    fill_dataloader,
                                                    self.device,
                                                    self._cfg,
                                                    monitor=self._monitor)
        sample_size, gad_para, results = self.trainer_gad.train()
        self.state = round
        ...
        logger.info(
            self._monitor.format_eval_res(results,
                                          rnd=self.state,forms='raw',return_raw=True,
                                          role='Client #{}'.format(self.ID)))
        self.comm_manager.send(
            Message(msg_type='gad_para',
                    sender=self.ID,
                    receiver=[sender],
                    state=self.state,
                    content=(sample_size, gad_para)))


    def callback_funcs_for_model_para(self, message: Message):
        round, sender, content = message.state, message.sender, message.content
        self.trainer_gad.update(content)
        self.state = round
        sample_size, clf_para, results = self.trainer_gad.train()
        if self._cfg.federate.share_local_model and not \
                self._cfg.federate.online_aggr:
            clf_para = copy.deepcopy(clf_para)
        round_formatted_results =  self._monitor.format_eval_res(results,
                                          rnd=self.state,forms='raw',return_raw=True,
                                          role='Client #{}'.format(self.ID))
        if self.state< self._cfg.federate.total_round_num+self._cfg.fedsageplus.fedgen_epoch:
            # formatted_eval_res[fpr, tpr] = 0, 0
            round_formatted_results['Results_raw']['test_fpr'] = 0
            round_formatted_results['Results_raw']['test_tpr'] = 0
        logger.info(round_formatted_results)
        self.comm_manager.send(
            Message(msg_type='gad_para',
                    sender=self.ID,
                    receiver=[sender],
