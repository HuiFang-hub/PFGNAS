...
class LocalGenTrainer(NodeFullBatchTrainer):
    ...
    def parse_data(self, data):
        ...
        # for i in data['data']:
        #     print(i)
        data_loader = NeighborLoader(data=data['data'], num_neighbors=[-1, -1],
        ...
        if isinstance(data, dict):
            for mode in ["train", "val", "test"]:
                # data =  NodeClassificationDataset(data)
                data_loader =  NeighborLoader(data = data['data'], num_neighbors=[-1,-1],batch_size=self.cfg.dataloader.batch_size)


                # DataLoader(data['data'], batch_size=self.cfg.dataloader.batch_size, shuffle=True)
                init_dict["{}_loader".format(mode)] = data_loader
                init_dict["{}_data".format(mode)] = None
                # For node-level task dataloader contains one graph
                init_dict["num_{}_data".format(mode)] = len(data_loader)
        else:
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        ctx.loss_batch = (self.cfg.fedsageplus.a * loss_num +
                          self.cfg.fedsageplus.b * loss_feat*1e6 +
    ...
    def update_by_grad(self, grads):
        ...
        for key in grads.keys():
            if isinstance(grads[key], list):
                ...
                grads[key] = grads[key].to(self.ctx.device)
        self.ctx.model= self.ctx.model.to(self.ctx.device)
        for key, value in self.ctx.model.named_parameters():
            # value.grad = value.grad.to(self.ctx.device)
            value.grad += grads[key].float()
        # grads = grads.to(ctx.device)
