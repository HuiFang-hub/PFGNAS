def wrap_attacker_trainer(base_trainer, config):
    ...
    Args:
        ...
        will be wrapped;
        config (src.federatedscope.core.configs.config.CN): the configure;
    ...
    if config.attack.attack_method.lower() == 'gan_attack':
        from src.federatedscope.attack.trainer import wrap_GANTrainer
        return wrap_GANTrainer(base_trainer)
    elif config.attack.attack_method.lower() == 'gradascent':
        from src.federatedscope.attack.trainer import wrap_GradientAscentTrainer
        return wrap_GradientAscentTrainer(base_trainer)
    elif config.attack.attack_method.lower() == 'backdoor':
        from src.federatedscope.attack.trainer import wrap_backdoorTrainer
