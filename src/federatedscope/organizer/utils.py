...
from src.federatedscope.core.cmd_args import parse_args
...
from src.federatedscope.organizer.cfg_client import env_name, root_path, fs_version

logger = logging.getLogger('src.federatedscope')
...
class SSHManager(object):
    ...
    def _exec_python_cmd(self, command):
        ...
        command = f'source ~/.bashrc; conda activate {env_name}; ' \
                  f'cd ~/{root_path}/src.federatedscope; {command}'
        _, stdout, stderr = self.ssh.exec_command(command)
        stdout = stdout.read().decode('ascii').strip("\n")
        stderr = stderr.read().decode('ascii').strip("\n")
        return stdout, stderr
    ...
    def _check_conda(self):
        ...
        output, err = self._exec_cmd(f'conda activate {env_name}; '
                                     f'python -c "import src.federatedscope; '
                                     f'print(src.federatedscope.__version__)"')
        if err:
            logger.error(err)
    ...
    def _check_source(self):
        ...
        """
        fs_path = os.path.join(root_path, 'src.federatedscope', '.git')
    ...
    def launch_task(self, command):
        ...
        stdout, _ = self._exec_python_cmd(f'nohup python '
                                          f'src.federatedscope/main.py {command} '
                                          f'> /dev/null 2>&1 & echo $!')
