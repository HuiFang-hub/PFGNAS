...
from src.autogl.module.nas.utils import pf_replace_layer_choice, replace_input_choice,serve_replace_layer_choice
from src.autogl.module.nas.algorithm.pfgnas import PFGNASLayerChoice,PFGNASInputChoice,pf_serve_LayerChoice
# from src.autogl.module.nas.algorithm.darts import
...
class BaseRunner(object):
    ...
    def _setup_client(self,
        ...
        client = self.client_class(ID=client_id,
                                   ...
                                   data=client_data,
                                   model=client_model
                                    or get_model(client_specific_config.model,self.cfg.device,
                                                client_data,
...
class StandaloneRunner(BaseRunner):
    def _set_up(self):
        ...
        self._shared_client_model = get_model(
            self.cfg.model,self.cfg.device,self.data[1], backend=self.cfg.backend
    ...
    def _get_server_args(self, resource_info=None, client_resource_info=None):
        if self.server_id in self.data:
            ...
            model = get_model(self.cfg.model, self.cfg.device,
                              server_data,
                              backend=self.cfg.backend)
...
class pf_StandaloneRunner(StandaloneRunner):
    def _set_up(self):
        self.is_run_online = True if self.cfg.federate.online_aggr else False
        self.shared_comm_queue = deque()
        ...
        # assume the client-wise data are consistent in their input&output
        ############# client model ############
       #['abd2-efh3-iab1', 'cde2-fhi3-jabc4', 'cdf5-ghj1-abc2', 'dfg2-hia3-bde4', 'efg4-ijb4-cef6',
       #'fgi1-jac2-dfg4', 'gij5-bcd5-efgh1', 'hia1-bde3-fgi4', 'iab5-cdf1-gij3', 'jbc3-dfg4-ija1']
        operations_list = self.cfg.model.operations.split('-') #operations =['abd2','egh3','iab1']
        for client_id,operations in zip(range(1, self.cfg.federate.client_num + 1),operations_list):
            # nas_modules = []
            operations = list(operations)
            self._shared_client_model = \
                get_model( self.cfg.model, self.cfg.device, self.data[1], backend=self.cfg.backend,operations=operations) \
                    if self.cfg.federate.share_local_model else None
            self._shared_client_model.instantiate()
            # client_model = copy.deepcopy(self._shared_client_model)
            # nas_modules = []
            pf_replace_layer_choice(self._shared_client_model, PFGNASLayerChoice,operations,self.cfg.device)
            replace_input_choice(self._shared_client_model, PFGNASInputChoice)
            # use alpha to change the forward of model
            # selection = self.export(nas_modules)
            # client_model=  self._shared_client_model.parse_model(selection,self.cfg.device)
            # client_model.initialize()
            # new_client_model =client_model.model
            # assert isinstance(new_client_model, torch.nn.Module)
    ...
    def _get_server_args(self, resource_info=None, client_resource_info=None):
        if self.server_id in self.data:
            ...
            model = get_model(self.cfg.model, self.cfg.device,
                              server_data,
                              backend=self.cfg.backend)
            serve_replace_layer_choice(model,pf_serve_LayerChoice, self.cfg.device)
            replace_input_choice(model, PFGNASInputChoice)
    ...
    def _setup_server(self, resource_info=None, client_resource_info=None):
        ...
        if self.mode == 'standalone':
            if self.server_id in self.data:
                ...
                data_representative = self.data[1]
                model = get_model(
                    self.cfg.model,self.cfg.device,
                    data_representative,
                    backend=self.cfg.backend
                )  # get the model according to client's data if the server
                # does not own data
    ...
    def _setup_client(self,
        ...
        if self.client_class:
            ...
            client = self.client_class(
                ...
                config=client_specific_config,
                data=client_data,
                model=client_model or get_model(client_specific_config.model,self.cfg.device,
                                                client_data,
                                                backend=self.cfg.backend),
                device=client_device,
