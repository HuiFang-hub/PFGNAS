...
class Monitor(object):
    ...
    def __init__(self, cfg, monitored_object=None):
        ...
        self.metric_calculator = MetricCalculator(cfg.eval.metrics,cfg.model.num_classes)
    ...
    def finish_fl(self):
        ...
        sys_metric_f_name = os.path.join(self.results_dir, "system_metrics.log")
        ...
        if file_io:
            ...
            if not os.path.exists(sys_metric_f_name):
                logger.warning(
                    ...
                    "$results_dir$/system_metrics.log, "
            ...
            if len(id_to_be_merged) != len(set(id_to_be_merged)):
                logger.warning(
                    ...
                    f"Plz check whether you specify the 'results_dir' "
    ...
    def save_formatted_results(self,
        ...
        if save_file_name != "":
            with open(os.path.join(self.results_dir, save_file_name),
                      "a") as outfile:
                outfile.write(line)
        if self.use_wandb and self.wandb_online_track:
            try:
    ...
    def finish_fed_runner(self, fl_mode=None):
        ...
        if self.use_wandb and not self.wandb_online_track:
            ...
            with open(os.path.join(self.results_dir, "eval_results.log"),
                      "r") as exp_log_f:
                ...
                sys_metric_f_name = os.path.join(self.results_dir,
                                                 "system_metrics.log")
                with open(sys_metric_f_name, "r") as f:
                    for line in f:
                        res = json.loads(line)
    ...
    def compress_raw_res_file(self):
        ...
        """
        old_f_name = os.path.join(self.results_dir, "eval_results.raw")
    ...
    def format_eval_res(self,
        ...
        with open(os.path.join(self.results_dir, "eval_results_raw.log"),
                  "a") as outfile:
