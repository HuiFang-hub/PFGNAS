...
class MetricCalculator(object):
    ...
    """
    def __init__(self, eval_metric: Union[Set[str], List[str], str],num_classes):
    ...
    def eval(self, ctx):
        ...
        for metric, (func, _) in self.eval_metric.items():
            results["{}_{}".format(ctx.cur_split,metric)] =\
                func(ctx=ctx,y_true=y_true,y_pred=y_pred,y_prob=y_prob, metric=metric,num_classes = self.num_classes)
        return results
...
def eval_acc(y_true, y_pred, num_classes, **kwargs):

    accuracy = Accuracy(task="multiclass", num_classes=num_classes,top_k = 1)
    res = accuracy(y_pred, y_true)
...
def eval_roc_auc(y_true, y_prob, **kwargs):
    ...
    return  round(res.item(), 4) #sum(rocauc_list) / len(rocauc_list)
...
def eval_loss(ctx, **kwargs):
    return round(ctx.loss_batch_total,4)


def eval_avg_loss(ctx, **kwargs):
    return round(ctx.loss_batch_total / ctx.num_samples,4)
