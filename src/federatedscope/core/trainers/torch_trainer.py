...
class GeneralTorchTrainer(Trainer):
    ...
    def update(self, model_parameters, strict=False):
        ...
        """
        if self.ctx.cfg.federate.method =='pFedMe':
            merged_param = []
            for parameters in model_parameters:
                for key in parameters:
                    parameters[key] = param2tensor(parameters[key])
                # new_para.append(parameters)
                merged_param = merge_param_dict(self.ctx.model.state_dict().copy(),
                                            self._param_filter(parameters))
                # merged_param.append(merge_param_dict(self.ctx.model.state_dict().copy(),
                #                             self._param_filter(parameters)))
        else:
            for key in model_parameters:
                model_parameters[key] = param2tensor(model_parameters[key])
            # Due to lazy load, we merge two state dict
            # test = self.ctx.model.state_dict().copy()   # 19
            # test2 = self._param_filter(model_parameters)  # 257
            merged_param = merge_param_dict(self.ctx.model.state_dict().copy(),
