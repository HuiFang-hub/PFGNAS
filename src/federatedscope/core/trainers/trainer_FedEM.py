...
class FedEMTrainer(GeneralMultiModelTrainer):
    ...
    def _hook_on_batch_start_track_batch_idx(self, ctx):
        ...
        if ctx.cur_mode == 'train':
            ctx.cur_batch_idx = (self.ctx.cur_batch_idx + 1) % self.ctx.num_train_batch
        else:
            ctx.cur_batch_idx = (self.ctx.cur_batch_idx + 1) % self.ctx.num_test_batch
    ...
    def _hook_on_fit_end_ensemble_eval(self, ctx):
        ...
        ctx.ys_prob_ensemble += torch.cat(ctx.ys_prob, dim=0) * self.weights_internal_models[
            ctx.cur_model_idx].item()
        ...
        if ctx.cur_model_idx == self.model_nums - 1:
            ...
            #                      LIFECYCLE.ROUTINE)
            ctx.ys_true = CtxVar(torch.cat(ctx.ys_true, dim=0),
                                 LIFECYCLE.ROUTINE)
