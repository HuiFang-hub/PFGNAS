...
from src.autogl.module.nas.space.graph_nas import GraphNasNodeClassificationSpace,PFGNASSpace
...
def get_shape_from_data(data, model_config, backend='torch'):
    ...
    if isinstance(data_representative, dict):
        ...
        if issubclass(type(data_representative), torch.utils.data.DataLoader):
            data= next(iter(data_representative))
            if isinstance(data, list):
                return data[0].x.shape
            return data.x.shape
        else:
            try:
...
def get_model(model_config, device,local_data=None, backend='torch',operations = None):
    """
    ...
    for func in register.model_dict.values():
        model = func(model_config, device)
    ...
    if model_config.type.lower() == 'lr':
        ...
        model = get_transformer(model_config, input_shape)
            ...
            'gcn', 'sage',  'gat', 'gin','gpr', 'fc','sgc', 'arma','appnp','mpnn'
    ]:
        # from src.federatedscope.gfl.model import get_gnn
        # model = get_gnn(model_config, input_shape,model_config.num_classes)
        from src.federatedscope.gfl.model import get_model
        model = get_model(model_name=model_config.type, model_config=model_config,
                          input_shape=input_shape[-1], out_shape=model_config.num_classes)
    ...
    elif model_config.type.lower() in ['fl-darts']:
        model = GraphNasNodeClassificationSpace(con_ops=['concat'])
        model.instantiate(input_dim=input_shape[-1], output_dim=model_config.num_classes)
    elif model_config.type.lower() in ['pfgnas-evo','pfgnas-random','pfgnas']:
        model = PFGNASSpace(model_config,operations, con_ops=['concat'])
        model.instantiate(input_dim=input_shape[-1], output_dim=model_config.num_classes)
    elif model_config.type.lower() == 'pfgnas-one':
        operations = model_config.operations
        from src.autogl.module.nas.algorithm.pfgnas import PFGNASLayerChoice, PFGNASInputChoice
        from src.autogl.module.nas.utils import pf_replace_layer_choice, replace_input_choice
        model = PFGNASSpace(model_config, operations, con_ops=['concat'])
        model.instantiate(input_dim=input_shape[-1], output_dim=model_config.num_classes)

        nas_modules = []
        pf_replace_layer_choice(model, PFGNASLayerChoice, operations, device,modules=nas_modules)
        replace_input_choice(model, PFGNASInputChoice,modules=nas_modules)

        selection = export(nas_modules)
        client_model=  model.parse_model(selection, device)
