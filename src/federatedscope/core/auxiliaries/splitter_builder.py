...
try:
    from src.federatedscope.contrib.splitter import *
except ImportError as error:
    logger.warning(
        f'{error} in `src.federatedscope.contrib.splitter`, some modules are not '
        f'available.')
...
def get_splitter(config):
    ...
    if config.data.splitter == 'lda':
        from src.federatedscope.core.splitters.generic import LDASplitter
    ...
    # graph splitter
        ...
        from src.federatedscope.core.splitters.graph import ScaffoldSplitter
        ...
        from src.federatedscope.core.splitters.graph import RandChunkSplitter
        splitter = RandChunkSplitter(client_num, **kwargs)
    elif config.data.splitter == 'iid':
        from src.federatedscope.core.splitters.generic import IIDSplitter
