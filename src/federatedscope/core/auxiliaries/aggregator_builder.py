...
def get_aggregator(method, model=None, device=None, online=False, config=None):
    ...
    if config.backend == 'tensorflow':
        from src.federatedscope.cross_backends import FedAvgAggregator
        return FedAvgAggregator(model=model, device=device)
    else:
        from src.federatedscope.core.aggregators import ClientsAvgAggregator, \
    ...
    if config.data.type.lower() == 'hetero_nlp_tasks' and \
        ...
        from src.federatedscope.nlp.hetero_tasks.aggregator import ATCAggregator
