...
def get_resfile_path(cfg):
    ...
    cfg.results_dir = os.path.join(cfg.results_DIR, cfg.expname)
    count = 0

    for root, dirs, files in os.walk(cfg.results_dir):
        for dir_name in dirs[:]:
            if dir_name not in ["checkpt", 'models'] :  # 使用切片复制dirs列表以便在循环中修改它
                dir_path = os.path.join(root, dir_name)
                if cfg.federate.method == 'FedPUB':
                    best_res_log_path = os.path.join(dir_path, "finally_res.log")
                else:
                    best_res_log_path = os.path.join(dir_path, "system_metrics.log")
                # 如果子文件夹中不存在system_metrics.log文件，则删除该子文件夹
                if not os.path.exists(best_res_log_path):
                    print(f"Deleting directory: {dir_path}")
                    shutil.rmtree(dir_path)
                else:
                    count += 1
    outdir = os.path.join(cfg.results_dir, "sub_exp" +
...
def update_logger(cfg, clear_before_add=False):
    ...
    count,cfg = get_resfile_path(cfg)

    # create file handler which logs even debugc messages
    fh = logging.FileHandler(os.path.join(cfg.results_dir, 'exp_print.log'))
    ...
    # root_logger.info(f"the output dir is {cfg.results_dir}")
